...
export default function BillPrint() {
    ...
    <div className="inv-cust-inv-row">
      ...
      <div className="inv-inv-box">
        ...
        <div className="cust-row">
          <div className="label">Bill Type</div>
          <div style={{ fontWeight: 600, textTransform: "capitalize" }}>
            {type === "pharmacy" ? "Pharmacy"
              : type === "lab" ? "Lab / Investigation"
              : type === "consultation" ? "Consultation"
              : type === "op" ? "OP Consolidated"
              : "IP Overall"}
          </div>
        </div>
        ...
        <div className="invoice-sheet">
          ...
          <table className="inv-items-table">
            ...
            <tfoot>
              <tr>
                ...
                <td>{bill.totalQty}</td>
                <td colSpan={3}></td>
                <td colSpan={2} className="text-right" style={{ fontWeight: 800 }}>Rs. {bill.grandTotal?.toFixed(2)}</td>
              </tr>
            </tfoot>
          </table>
          ...
          <div style={{ display: "flex", justifyContent: "flex-end", padding: "8px 16px" }}>
            <table style={{ borderCollapse: "collapse", fontSize: 13, minWidth: 260 }}>
              <tbody>
                <tr><td style={{ padding: "4px 16px 4px 4px", color: "#6b7280" }}>Subtotal</td><td style={{ textAlign: "right", fontWeight: 600 }}>Rs. {bill.totalTaxableValue?.toFixed(2)}</td></tr>
                <tr><td style={{ padding: "4px 16px 4px 4px", color: "#6b7280" }}>IGST</td><td style={{ textAlign: "right", fontWeight: 600 }}>Rs. {bill.totalIgstAmount?.toFixed(2)}</td></tr>
                {bill.amountPaid > 0 && <tr><td style={{ padding: "4px 16px 4px 4px", color: "#166534" }}>Amount Paid</td><td style={{ textAlign: "right", fontWeight: 600, color: "#166534" }}>Rs. {Number(bill.amountPaid).toFixed(2)}</td></tr>}
                <tr style={{ borderTop: "2px solid #1a2b3c" }}>
                  <td style={{ padding: "6px 16px 4px 4px", fontWeight: 800, fontSize: 15 }}>Grand Total</td>
                  <td style={{ textAlign: "right", fontWeight: 800, fontSize: 15 }}>Rs. {bill.grandTotal?.toFixed(2)}</td>
                </tr>
              </tbody>
            </table>
          </div>
          ...
          <table className="inv-items-table">
            ...
            <tfoot>
              <tr>
                ...
                <td className="text-right" style={{ fontWeight: 800 }}>Rs. {bill.grandTotal?.toFixed(2)}</td>
              </tr>
            </tfoot>
          </table>
  ...
  // ── OP CONSOLIDATED PRINT ────────────────────────────────────────────────
  if (type === "op") {
    const subtotal = (bill.opSections || []).reduce((a, s) => a + (Number(s.sectionTotal) || 0), 0);
    const discount = Number(bill.opDiscount) || 0;
    const balance  = Math.max(0, (Number(bill.grandTotal) || 0) - (Number(bill.amountPaid) || 0));
    ...
    return (
      <div className="invoice-page-wrap">
        ...
        <div className="invoice-sheet">
          ...
          <div className="inv-gst-row">
            <div className="gst-left">GSTIN: {settings.gstNumber}</div>
            <div className="tax-invoice-title">OP CONSOLIDATED BILL</div>
            <div className="original-recipient">ORIGINAL FOR RECIPIENT</div>
          </div>
          <PatientBox />

          {/* One table per section */}
          {(bill.opSections || []).map((section, si) => (
            <div key={si} style={{ marginBottom: 12 }}>
              ...
              <table className="inv-items-table" style={{ marginTop: 0 }}>
                <thead>
                  <tr>
                    <th>Sr.</th>
                    <th className="text-left">Description</th>
                ...
                <tfoot>
                  <tr>
                    <td colSpan={5} className="text-right" style={{ fontWeight: 700 }}>
                      {section.sectionLabel} Total
                    </td>
                    <td className="text-right" style={{ fontWeight: 800 }}>
                      Rs. {Number(section.sectionTotal).toFixed(2)}
                    </td>
                  </tr>
                </tfoot>
              </table>
          ...
          {/* Notes */}
          {bill.opNotes && (
            <div style={{ padding: "6px 10px", fontSize: 12, color: "#374151", borderTop: "1px solid #e5e7eb" }}>
              <strong>Notes:</strong> {bill.opNotes}
            </div>
          )}

          <div className="inv-words-row">
            <div>Amount in Words: <strong>{bill.amountInWords}</strong></div>
            <div>(E &amp; O.E.)</div>
          </div>
              ...
              <tbody>
                ...
                {(bill.opSections || []).map((section, si) =>
                  Number(section.sectionTotal) > 0 ? (
                    <tr key={si}>
                      <td style={{ padding: "3px 16px 3px 4px", color: "#6b7280" }}>{section.sectionLabel}</td>
                      <td style={{ textAlign: "right", fontWeight: 600 }}>Rs. {Number(section.sectionTotal).toFixed(2)}</td>
                    </tr>
                  ) : null
                )}
                <tr style={{ borderTop: "1px solid #d1d5db" }}>
                  <td style={{ padding: "4px 16px 4px 4px", color: "#6b7280" }}>Subtotal</td>
                  <td style={{ textAlign: "right", fontWeight: 600 }}>Rs. {subtotal.toFixed(2)}</td>
                </tr>
                {discount > 0 && (
                  <tr>
                    <td style={{ padding: "4px 16px 4px 4px", color: "#b91c1c" }}>Discount</td>
                    <td style={{ textAlign: "right", fontWeight: 600, color: "#b91c1c" }}>- Rs. {discount.toFixed(2)}</td>
                  </tr>
                )}
                <tr style={{ borderTop: "2px solid #1a2b3c" }}>
                  <td style={{ padding: "6px 16px 4px 4px", fontWeight: 800, fontSize: 15 }}>Grand Total</td>
                  <td style={{ textAlign: "right", fontWeight: 800, fontSize: 15 }}>Rs. {Number(bill.grandTotal).toFixed(2)}</td>
                </tr>
                {Number(bill.amountPaid) > 0 && (
                  <tr>
                    <td style={{ padding: "4px 16px 4px 4px", color: "#166534" }}>Amount Paid</td>
                    <td style={{ textAlign: "right", fontWeight: 600, color: "#166534" }}>Rs. {Number(bill.amountPaid).toFixed(2)}</td>
                  </tr>
                )}
                {balance > 0 && (
                  <tr>
                    <td style={{ padding: "4px 16px 4px 4px", color: "#b91c1c", fontWeight: 700 }}>Balance Due</td>
                    <td style={{ textAlign: "right", fontWeight: 700, color: "#b91c1c" }}>Rs. {balance.toFixed(2)}</td>
                  </tr>
                )}
              </tbody>
            </table>
  ...
  return (
    <div className="invoice-page-wrap">
      ...
      <div className="invoice-sheet">
        ...
        <div style={{ display: "flex", gap: 24, borderTop: "1.5px solid #1a2b3c", borderBottom: "1.5px solid #1a2b3c", padding: "8px 10px", fontSize: 12.5, flexWrap: "wrap", margin: 0 }}>
        ...
        <div style={{ display: "flex", justifyContent: "flex-end", padding: "8px 16px" }}>
          <table style={{ borderCollapse: "collapse", fontSize: 13, minWidth: 280 }}>
            <tbody>
              <tr><td style={{ padding: "4px 16px 4px 4px", color: "#6b7280" }}>Hospital Charges</td><td style={{ textAlign: "right", fontWeight: 600 }}>Rs. {(bill.ipItems?.reduce((s, it) => s + Math.max(0, (Number(it.qty) || 0) * (Number(it.rate) || 0) - ((Number(it.qty) || 0) * (Number(it.rate) || 0) * (Number(it.discountPercent) || 0) / 100)), 0)).toFixed(2)}</td></tr>
              {Number(bill.ipConsultFee) > 0 && <tr><td style={{ padding: "4px 16px 4px 4px", color: "#6b7280" }}>Consultation Fee</td><td style={{ textAlign: "right", fontWeight: 600 }}>Rs. {Number(bill.ipConsultFee).toFixed(2)}</td></tr>}
              {Number(bill.ipPharmacyTotal) > 0 && <tr><td style={{ padding: "4px 16px 4px 4px", color: "#6b7280" }}>Pharmacy</td><td style={{ textAlign: "right", fontWeight: 600 }}>Rs. {Number(bill.ipPharmacyTotal).toFixed(2)}</td></tr>}
              {Number(bill.ipLabTotal) > 0 && <tr><td style={{ padding: "4px 16px 4px 4px", color: "#6b7280" }}>Lab / Investigation</td><td style={{ textAlign: "right", fontWeight: 600 }}>Rs. {Number(bill.ipLabTotal).toFixed(2)}</td></tr>}
              {Number(bill.ipDiscount) > 0 && <tr><td style={{ padding: "4px 16px 4px 4px", color: "#b91c1c" }}>Discount</td><td style={{ textAlign: "right", fontWeight: 600, color: "#b91c1c" }}>- Rs. {Number(bill.ipDiscount).toFixed(2)}</td></tr>}
              {Number(bill.ipAdvancePaid) > 0 && <tr><td style={{ padding: "4px 16px 4px 4px", color: "#166534" }}>Advance Paid</td><td style={{ textAlign: "right", fontWeight: 600, color: "#166534" }}>- Rs. {Number(bill.ipAdvancePaid).toFixed(2)}</td></tr>}
              {bill.amountPaid > 0 && <tr><td style={{ padding: "4px 16px 4px 4px", color: "#166534" }}>Amount Paid</td><td style={{ textAlign: "right", fontWeight: 600, color: "#166534" }}>Rs. {Number(bill.amountPaid).toFixed(2)}</td></tr>}
              <tr style={{ borderTop: "2px solid #1a2b3c" }}>
